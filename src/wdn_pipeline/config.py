...
random seed, fault injections (placeholders until Phases 3 and 4) and
output settings. This module loads, validates and exposes those settings
as immutable typed objects.
"""
...
class FaultsConfig(_Frozen):
    """Placeholder for fault injection.

    The fields are typed as lists of opaque dicts for now. Phase 3 (leak)
    and Phase 4 (sensor faults) will replace these with strict typed
    submodels.
    """
...
class ScenarioConfig(_Frozen):
    ...
    For Phase 2 only ``normal`` is supported. ``label`` flows into both
    output filenames and the per-timestep label column.
    """
