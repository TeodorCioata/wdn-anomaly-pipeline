...
Phase 5 Week 8. Emits a 24h / 1h-timestep PDD normal-scenario YAML
...
The pipeline always uses the pure-Python WNTRSimulator,
...
def skip_reason(stats: dict, max_junctions: int) -> str | None:
    ...
    Two pre-known WNTRSimulator incompatibilities are caught here:
    ...
    if stats["headloss"] != "H-W":
        return f"uses {stats['headloss']} headloss, unsupported by WNTRSimulator"
    if max_junctions and stats["junctions"] > max_junctions:
        return f"{stats['junctions']} junctions > cap {max_junctions} (too slow for WNTRSimulator)"
    return None
...
def _config_header(path: Path, stats: dict, mode: str, seed: int) -> str:
    return (
        f"# Sweep config (Phase 5 Week 8) generated by "
