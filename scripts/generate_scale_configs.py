"""Generate the Phase 5 Week 8 scale-run scenario configs (WP4).
...
def build_scenarios(
    ...
    for net_name, inp_path, mode in NETWORKS:
        ...
        for i in range(n_normal):
            cfg = _base_config(
                net_name, inp_path, mode, seed=i, label=f"normal_{i}",
        ...
        for i in range(n_leak):
            ...
            cfg = _base_config(
                net_name, inp_path, mode, seed=seed, label=f"leak_{i}",
                scenario_type="leak",
            )
        ...
        for i in range(n_sensor):
            ...
            seed = int(rng.integers(0, 2**31 - 1))
            cfg = _base_config(
                net_name, inp_path, mode, seed=seed,
                label=f"sensor_{ftype}_{i}", scenario_type="sensor_fault",
            )
        ...
        for i in range(n_cumulative):
            ...
            cfg = _base_config(
                net_name, inp_path, mode, seed=seed,
                label=f"cumulative_{i}", scenario_type="cumulative",
            )
            cfg["faults"] = {
...
def _write(out_dir: Path, net_name: str, label: str, cfg: dict, master_seed: int) -> None:
    ...
    header = (
        f"# Scale-run config (Phase 5 Week 8, WP4) generated by "
